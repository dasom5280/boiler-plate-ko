...
export function loginUser(dateToSubmit) {
    const request = axios.post('/api/users/login',dateToSubmit).then(response => response.data)
...
export function registerUser(dateToSubmit) {
    const request = axios.post('/api/users/register',dateToSubmit).then(response => response.data)
